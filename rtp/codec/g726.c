	...
	int		srn)
{
	...
	anexp = g726_quan(anmag, power2, 15) - 6;
	anmant = (anmag == 0) ? 32 :
 ...
 */
void
g726_init_state(struct g726_state_s *state_ptr)
{
	int		cnta;
	...
	struct g726_state_s *state_ptr)
{
	...
	sezi = g726_fmult(state_ptr->b[0] >> 2, state_ptr->dq[0]);
	for (i = 1; i < 6; i++)			/* ACCUM */
		sezi += g726_fmult(state_ptr->b[i] >> 2, state_ptr->dq[i]);
	return (sezi);
}
/*
 * predictor_pole()
...
g726_predictor_pole(
	struct g726_state_s *state_ptr)
{
	return (g726_fmult(state_ptr->a[1] >> 2, state_ptr->sr[1]) +
	    g726_fmult(state_ptr->a[0] >> 2, state_ptr->sr[0]));
}
/*
 * step_size()
 *
	...
	int *	qtab)
{
	...
	id = g726_quantize(dx, y, qtab, sign - 1);
