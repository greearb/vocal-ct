	...
	struct g726_state_s *state_ptr)
{
	...
	i = g726_quantize(d, y, qtab_723_40, 15);	/* i = ADPCM code */

	dq = g726_reconstruct(i & 0x10, _dqlntab[i], y);	/* quantized diff */

	sr = (dq < 0) ? se - (dq & 0x7FFF) : se + dq; /* reconstructed signal */

	dqsez = sr + sez - se;		/* dqsez = pole prediction diff. */

	g726_update(5, y, _witab[i], _fitab[i], dq, sr, dqsez, state_ptr);
	...
	struct g726_state_s *state_ptr)
{
	...
	y = g726_step_size(state_ptr);	/* adaptive quantizer step size */
	dq = g726_reconstruct(i & 0x10, _dqlntab[i], y);	/* estimation diff. */
	...
	switch (out_coding) {
	case AUDIO_ENCODING_ALAW:
		return (g726_tandem_adjust_alaw(sr, se, y, i, 0x10, qtab_723_40));
	case AUDIO_ENCODING_ULAW:
		return (g726_tandem_adjust_ulaw(sr, se, y, i, 0x10, qtab_723_40));
	case AUDIO_ENCODING_LINEAR:
