	...
	struct g726_state_s *state_ptr)
{
	...
	sezi = g726_predictor_zero(state_ptr);
	sez = sezi >> 1;
	se = (sezi + g726_predictor_pole(state_ptr)) >> 1;	/* estimated signal */

	d = sl - se;				/* estimation difference */

	/* quantize the prediction difference */
	y = g726_step_size(state_ptr);		/* quantizer step size */
	i = g726_quantize(d, y, qtab_721, 7);	/* i = ADPCM code */

	dq = g726_reconstruct(i & 8, _dqlntab[i], y);	/* quantized est diff */

	sr = (dq < 0) ? se - (dq & 0x3FFF) : se + dq;	/* reconst. signal */

	dqsez = sr + sez - se;			/* pole prediction diff. */

	g726_update(4, y, _witab[i] << 5, _fitab[i], dq, sr, dqsez, state_ptr);
	...
	struct g726_state_s *state_ptr)
{
	...
	sezi = g726_predictor_zero(state_ptr);
	sez = sezi >> 1;
	sei = sezi + g726_predictor_pole(state_ptr);
	se = sei >> 1;			/* se = estimated signal */

	y = g726_step_size(state_ptr);	/* dynamic quantizer step size */

	dq = g726_reconstruct(i & 0x08, _dqlntab[i], y); /* quantized diff. */
	...
	switch (out_coding) {
	case AUDIO_ENCODING_ALAW:
		return (g726_tandem_adjust_alaw(sr, se, y, i, 8, qtab_721));
	case AUDIO_ENCODING_ULAW:
		return (g726_tandem_adjust_ulaw(sr, se, y, i, 8, qtab_721));
	case AUDIO_ENCODING_LINEAR:
