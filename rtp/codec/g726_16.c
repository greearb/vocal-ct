	...
	struct g726_state_s *state_ptr)
{
	...
	i = g726_quantize(d, y, qtab_723_16, 1);  /* i = ADPCM code */
	...
	dq = g726_reconstruct(i & 2, _dqlntab[i], y); /* quantized diff. */

	sr = (dq < 0) ? se - (dq & 0x3FFF) : se + dq; /* reconstructed signal */

	dqsez = sr + sez - se;		/* pole prediction diff. */

	g726_update(2, y, _witab[i], _fitab[i], dq, sr, dqsez, state_ptr);
	...
	struct g726_state_s *state_ptr)
{
	...
	dq = g726_reconstruct(i & 0x02, _dqlntab[i], y); /* unquantize pred diff */
	...
	switch (out_coding) {
	case AUDIO_ENCODING_ALAW:
		return (g726_tandem_adjust_alaw(sr, se, y, i, 2, qtab_723_16));
	case AUDIO_ENCODING_ULAW:
		return (g726_tandem_adjust_ulaw(sr, se, y, i, 2, qtab_723_16));
	case AUDIO_ENCODING_LINEAR:
