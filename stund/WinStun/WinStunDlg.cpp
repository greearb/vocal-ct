// WinStunDlg.cpp : implementation file
//
...
void CWinStunDlg::OnBnClickedTest()
{
	...
	case StunTypeConeNat:
		text = _T("Cone Nat detect");
		break;
	case StunTypeRestrictedNat:
		text = _T("IP restricted NAT detected");
		//clog << "Restricted Nat";
		break;
	case StunTypePortRestrictedNat:
		text = _T("Port restricted NAT detected");
		//clog << "Port Restricted Nat";
		break;
	case StunTypeSymNat:
		text = _T("Port restriced NAT without port reuse - STUN will not fix for");
		break;
	case StunTypeSymFirewall:
		text = _T("Symetric firewall");
